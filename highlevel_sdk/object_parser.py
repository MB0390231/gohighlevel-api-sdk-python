...
class ObjectParser(object):
    ...
    def parse_multiple(response, target_class=None, token_data=None):
        ...
        for key in response.keys():
            if key == "meta" or key == "traceId":
                continue

            if isinstance(response[key], list):
                for json_obj in response[key]:
                    ret.append(ObjectParser.parse_single(json_obj, target_class, token_data))
            else:
                ret.append(ObjectParser.parse_single(response[key], target_class, token_data))
        return ret
